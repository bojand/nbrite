...
Builder.prototype.buildAction = function (action) {
  ...
  var properties = action.schema ? action.schema.properties : action.properties;
  var requiredProps = action.schema ? action.schema.required : action.required;

  var objPath = action.href;
  if (action.rel && action.rel === 'parent-action') {
    objPath = objPath.substring(0, objPath.lastIndexOf('/'));
  }

  var constructor = pathProxy.pathProxy(this.baseObj, objPath);
  ...
  constructor.prototype[getName(actionName)] = function (data, fn) {
    ...
    if (this.params.length !== pathParams.length) {
      var valErr = new Error('Invalid number of params in path (expected ' + pathParams.length + ', got ' + this.params.length + ').');
      return rejectError(valErr, deferred, fn);
    }
    ...
    for (var key in properties) {
      if (properties.hasOwnProperty(key) && data && data[key]) {
        var type = properties[key].type;
        var dataType = Array.isArray(data[key]) ? 'array' : typeof data[key];
        if (Array.isArray(type)) {
          // fix up the property type array
          type = type.map(function (it) {
            it = it.toLowerCase();
            if (it === 'integer') {
              it = 'number';
            }
          ...
          // fix up the type of the property
